...
| `RELAY_URL` | Relay WebSocket URL (worker config) | `wss://your-dashboard/relay/ws` |
...
# e.g. from the bot database or a CSV
...
**Known limitation**: The relay worker can only reach the subnet it's deployed on. Students on other internal subnets cannot be reached — their deployment checks must be passed manually via the dashboard or DB.
...
The relay worker bridges the deploy server to an internal network. It runs on a VM inside the network and handles HTTP and SSH check jobs for internal IPs that the bot can't reach directly.
...
│   └─ WebSocket ────────────────────────┐                   │
└────────────────────────────────────────│───────────────────┘
                                         │ wss://$DASHBOARD_URL/relay/ws
┌─ Internal VM ─────────────────────────│───────────────────┐
│  Relay Worker (systemd service)  ←────┘                    │
│   ├─ HTTP jobs: curl to internal IPs                       │
│   └─ SSH jobs: ssh -i key user@student-vm "command"        │
└────────────────────────────────────────────────────────────┘
```

### Resilience

Unstable networks periodically kill WebSocket connections. The system handles this:
...
The worker runs on the internal VM as a systemd service. Configure via environment variables:
- `RELAY_TOKEN` — shared secret for authentication
- `RELAY_URL` — WebSocket URL to the dashboard
- `SSH_KEY_PATH` — path to SSH private key for connecting to student VMs
...
- SSH key has no sudo access on student VMs
- Timeouts capped at 30 seconds, response bodies capped at 4KB

## Production Deployment

See `deploy/` directory for Docker Compose setup and `.env.example` for required environment variables.
...
**Gotcha**: `docker compose up -d` without a preceding `docker compose build` reuses old images. Always build first.

### Docker volumes and mounts

| Volume / Mount | Container | Contents |
|---|---|---|
| `bot-data` → `/app/data/` | bot, dashboard | `bot.db` (SQLite — users, attempts, results) |
...
| `$SANDBOX_DIR` | bot | Shared temp dir for clone_and_run repos |
